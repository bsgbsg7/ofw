...
# ========== 复杂度正则化参数 ==========
LAMBDA_COMPLEXITY = 0.5    # 复杂度惩罚系数 (λ=0.05无效，改为0.5)
                            # 越大 → 模型越偏好简单 Q (TDM-like)
                            # 越小 → 模型越能容忍复杂 Q (OFDM-like)

def compute_Q_complexity(Q):
    r"""
    计算 Q 矩阵的复杂度：每行能量分布的熵均值。

    物理直觉：
      - Q ≈ I (TDM)：每行能量集中在单个时隙 → 低熵 → 低复杂度
      - Q ≈ IFFT (OFDM)：每行能量均匀分散到所有时隙 → 高熵 → 高复杂度

    Args:
        Q: [batch, N, N] 复数矩阵

    Returns:
        mean_entropy: [batch] 每个样本的 Q 复杂度
    """
    # |Q[i,j]|^2 → 每行能量分布
    energy = tf.abs(Q)**2                                              # [batch, N, N]
    # 行内归一化: 每行的能量和 = 1
    row_sum = tf.reduce_sum(energy, axis=-1, keepdims=True)            # [batch, N, 1]
    normalized = energy / (row_sum + 1e-8)                             # [batch, N, N]
    # 每行的熵: -sum(p * log(p))
    row_entropy = -tf.reduce_sum(
        normalized * tf.math.log(normalized + 1e-8), axis=-1)          # [batch, N]
    # batch 内平均
    mean_entropy = tf.reduce_mean(row_entropy, axis=-1)                # [batch]
    return mean_entropy

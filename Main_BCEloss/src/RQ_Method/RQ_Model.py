...
from src.RQ_Method.RQ_creator_layer import RQ_creator_layer, RQ_creator_conv_gru
from src.RQ_Method.Q_Modulator import Q_Modulator
from src.RQ_Method.Q_Demodulator import Q_Demodulator
from utils.PAPR import emprical_papr

import matplotlib.pyplot as plt

class RQ_MODEL(keras.Model):
    ...
    def __init__(self,
        ...
        self._epsilon_P = 9               # [dB]        
        self._lamda_P = 0.0
        self._eta     = 1e-3
        
        # Layers
        # self._RQ_creator_layer = RQ_creator_layer(self._fft_size, self.Q_as_ifft)
        self._RQ_creator_layer = RQ_creator_conv_gru(self._fft_size)
    ...
    def call(self, batch_size, ebno_db):
        ...
        # channel input as time domian to Q_creator     
            ...
            paddings = tf.constant([[0, 0], [0, 0], [0, 0], [0,   self.OFDM_modulator(x_rg).shape[-1] - x_time_for_csi.shape[-1]]])
            x_time_for_csi = tf.pad(x_time_for_csi, paddings, mode='CONSTANT', constant_values=tf.complex(0.0, 0.0))
            y_time = self._channel_time(x_time_for_csi, h_time, 0)
            ...
            pilots_reference = x_time_for_csi[:,0,0,:self._fft_size]
            pilots_post_channel = tf.stack([pilots_post_channel, pilots_reference], axis=2)
        
        # RQ = self._RQ_creator_layer(pilots_post_channel, training=self.training)
        # Q = RQ[...,0]; R = RQ[...,1]
        Q, R = self._RQ_creator_layer(pilots_post_channel, training=self.training)
        ...
        # Q Demodultion
        r_freq = self._Q_demodulator(R, y_time)
        
        if self.visulaize_progress:
            self.visulaize(h_time, Q)
        ...
        if self.Q_as_ifft: 
            ...
            # count on RQ to equalize (only reshaping and csi symbol removal are needed)
        ...
        if self.training:
            ...
            total_loss = 1.0 * bce_loss + 0.0 * PAR_loss + 0.0 * PAR_loss_sqeure
            
            self._eta *= 1.001
            # if PAR_loss < 0.00000001:
            #     self._eta = 1.1            
    ...
    def visulaize(self, h_true, Q):
        ...
        plt.tight_layout()
        plt.savefig('_RQ_functions_Time_Visualization.png')
        plt.close()
        ...
        plt.grid(True)
        plt.savefig('_RQ_functions_Freq_Visualization.png')
        plt.close()
        pass
...
if __name__ == "__main__":
    sn.config.seed = SEED
    model = RQ_MODEL(training=True, visulaize=True)
    model(10,0)
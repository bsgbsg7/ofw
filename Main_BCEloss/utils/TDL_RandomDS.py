...
class TDL_RandomDS(ChannelModel):
    ...
    def __init__(   self,
        ...
        self._delay_spread_min = tf.constant(delay_spread_min, self.rdtype)
        self._delay_spread_max = tf.constant(delay_spread_max, self.rdtype)
    ...
    @property
    def delay_spread_min(self):
        r"""RMS delay spread [s]"""
        return self._delay_spread_min
    
    @property
    def delay_spread_max(self):
        r"""RMS delay spread [s]"""
        return self._delay_spread_max
    ...
    def __call__(self, batch_size, num_time_steps, sampling_frequency):
        ...
        if self._scale_delays:
            ...
            delays = self._delays
            # random scaling the delays
            delays = insert_dims(delays, 3, 0)
            min_val_nano =  int(self._delay_spread_min * 1e9)
            max_val_nano =  int(self._delay_spread_max * 1e9)
            rand_delay_spread_nano = tf.stop_gradient( config.tf_rng.uniform((batch_size, 1, 1, 1), minval=min_val_nano, maxval=max_val_nano, dtype=tf.int32) )
            rand_delay_spread = tf.cast(rand_delay_spread_nano, dtype=self.rdtype) * 1e-9
            delays = rand_delay_spread * delays
            # self._rand_delay_spread = rand_delay_spread
        else:
...
if __name__ == "__main__":
    ...
    # Delta input
    channel_in = tf.scatter_nd(indices=[[0, 0, 0, 0]] , updates=tf.constant([1.0 + 0j], dtype=tf.complex64), shape=(32, 1, 1, num_time_samples)) 
    ...
    plt.stem(np.abs(h_time[15,0,0,0,0,0]))
    # plt.xlim(-1,20)
    plt.xlabel(r"Time step $\ell$")
    plt.ylabel(r"$|\bar{h}|$");
    plt.savefig('_CIR_debug.png')

    ### plot few h_time with diferent delay spread ##
    num_time_samples = int(10000)  # Number of time samples input
    bandwidth = 5e6
    l_min, l_max = time_lag_discrete_time_channel(bandwidth)

    a, tau = channel_model_to_use(64, num_time_samples+l_min+l_max-1,bandwidth)
    h_time = cir_to_time_channel(bandwidth, a, tau,
                                    l_min=l_min, l_max=l_max, normalize=True) 
    for i in range(4):
        # channel Impulse response
        plt.figure()
        plt.title(f"Discrete-time channel impulse response DS of {tf.squeeze(channel_model_to_use._rand_delay_spread)[i].numpy()*1e9:.0f} [nano Sec]")
        plt.stem(np.abs(h_time[i,0,0,0,0,0]))
        # plt.xlim(-1,20)
        plt.xlabel(r"Time step $\ell$")
        plt.ylabel(r"$|\bar{h}|$");
        plt.savefig(f'_CIR_debug{i}.png')
        plt.close()
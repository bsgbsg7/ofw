...
class Q_Demodulator(Block):
    ...
    def build(self, Q, input_shape): # pylint: disable=unused-argument
    ...
    def call(self, Q, inputs):
        ...
        if len(self.cyclic_prefix_length.shape)==0:
            ...
            if self.l_min != 0:
                x = x[...,self.cyclic_prefix_length + self.l_min + self._fftp_bakcoff: self.l_min + self._fftp_bakcoff] # so that "future reflections" wont get into the currect ofdm symbol
            else:
        ...
        # Compute FFT
        # x = tf.tensordot(x, tf.transpose(tf.math.conj(Q)), axes=[-1, 0])

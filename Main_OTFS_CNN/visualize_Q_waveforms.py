#!/usr/bin/env python3
"""
Q 矩阵波形可视化 — CNN Receiver 版本
=====================================
三列: 平坦 3m/s | 多径 3m/s (620ns) | 多径 120m/s (620ns)
四行: CIR 输入 | CIR 时间演化 | Q 时域基底 | Q 频谱

与 Main_OTFS 版本的区别：加载 CNN receiver 模型的权重。
Q-creator 架构完全相同，因此可视化方式不变。
"""
...
# ================================================================
# Load CNN Receiver model (only difference from Main_OTFS version)
# ================================================================
from src.qQ_Method.qQ_Model_TV_CNN import qQ_MODEL_TV_CNN
model = qQ_MODEL_TV_CNN(training=False)
model(2, 40.0)
with open('weights-qQ_Method_TV_CNN', 'rb') as f:
    model.set_weights(pickle.load(f))
print("Model loaded (CNN Receiver).")
...
for col, (name, _, _, _) in enumerate(scenarios):
    ...
    ax.set_title(f'CIR input: {name}\n{l_tot} taps x {n_snap} snapshots, RMS_DS={rms:.2f}',
                 fontsize=10, fontweight='bold')
    plt.colorbar(im, ax=ax, shrink=0.85)

# ---- Row 1: CIR time evolution ----
for col, (name, _, _, _) in enumerate(scenarios):
    d = all_data[name]
    cir0 = np.abs(d['cir'][d['idx_max']])
...
plt.suptitle('CIR Input -> Q Waveform | CNN Receiver | DS: 10ns / 620ns / 620ns',
...
for col, (name, _, _, _) in enumerate(scenarios):
    ...
    ax.grid(True, alpha=0.3)
plt.suptitle('CIR Delay Profile (averaged over 12 snapshots) | CNN Receiver',
